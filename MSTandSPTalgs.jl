...
Se PQ viene realizzata tramite uno heap binario:
- Inizializzare la coda: $O(n)$
- Il **while** principale viene eseguito n volte ed ogni operazione deleteMin() è $O(log n)$, quindi $O(n log n)$
- Il ciclo interno viene eseguito $O(m)$ volte
- L'operazione decreaseKey() sullo heap che costa $O(log n)$

In totale quindi avremo una complessità di $O(m log n)$
